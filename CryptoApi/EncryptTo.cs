...
public class EncryptTo{
 ...
 public static void Main(String[] args)
	{
	String contentfile, encryptedContent, encryptedKey, encryptedIV;
	String subjectstr;

	EncryptTo oEnc = new EncryptTo();
	...
	if (!File.Exists(contentfile)){
	 Console.WriteLine("File '{0}' not found.", contentfile);
	 return;
	}

	encryptedContent	= args[1];
	encryptedKey		= args[2];
	encryptedIV		= args[3];

	Console.Write("Enter SubjectName of recipient, or filename of recipient certificate: ");
	subjectstr = Console.ReadLine();

	//----- Either a valid certificate file passed, or a cert store search string ----
	if (File.Exists(subjectstr))
	  oEnc.recipcert = oEnc.GetRecipientFileCert(subjectstr);
	else
	  oEnc.recipcert = oEnc.GetRecipientStoreCert(subjectstr);

	
	if(oEnc.recipcert == null){
	  Console.WriteLine("Couldn't find valid certificate in store or file matching '{0}'", subjectstr);
	  return;
	}

	//----- get recipient certificate public key parameters ----
	if(!oEnc.GetCertPublicKey(oEnc.recipcert)){
	 Console.WriteLine("Couldn't get recipient certificate public key");
	 return;
	}
	Console.WriteLine("Public key size:   {0} bits", oEnc.certkeysize);

	//---- ToDo: get cert SHA1 hash and save to file; better way to retrieve cert ----
	//byte[] certhash = oEnc.recipcert.GetCertHash(); //SHA1 hash
	//if(certhash.Length==20)
	//  oEnc.PutFileBytes("certhash", certhash, 20);

	//----- encrypt content with 3DES; shroud the 3DES Key and IV with RSA ----
	 if(oEnc.TripleDESEncrypt(contentfile, encryptedContent, encryptedKey, encryptedIV))
	   Console.WriteLine("\nSuccessfully encrypted '{0}' to '{1}'", contentfile, encryptedContent);
	 else
